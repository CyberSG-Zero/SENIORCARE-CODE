...
function evaluateChoice() {
    ...
    const namedog = localStorage.getItem('dogName') || 'Manolo';
    let resultHead = '';
    let resultText = '';
    let recommentMe = '';
    let accuracy = '';
    ...
    if (gameState.totalPoints >= 25 ) {
        ...
        resultText = `<span class='span-bold'>${namedog}</span> está feliz con la cantidad de comida que le diste. Has demostrado que conoces bien a tu perro. Mantener una <span class='span-bold'>porción adecuada</span> ayuda a tu perro a mantener un peso saludable, lo cual es crucial para su bienestar en la vejez. <br><br>Un peso equilibrado <span class='span-bold'>reduce el estrés</span> en sus articulaciones y órganos internos, contribuyendo a una mejor calidad de vida.`;
